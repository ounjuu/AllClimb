...
// 박스 9개
const renderData = (dataType, withObserver = false) => {
  boxContainer.innerHTML = ""; // 기존 콘텐츠 제거

  dataType.forEach((item, index) => {
    const div = document.createElement("div");
    div.className = "imgBox";
    div.onclick = () => {
      if (item.url) window.location.href = item.url; // 클릭 이벤트 처리
    };
  ...
  if (withObserver) initializeObserver(); // 옵저버 초기화
};

const moveHeader = (type) => {
  let dataType;

  if (type === "전체") dataType = data;
  else if (type === "Event") dataType = data2;
  else if (type === "Class") dataType = data3;
  else if (type === "New Gym") dataType = data4;

  renderData(dataType, true); // 옵저버 활성화 포함 렌더링
};
